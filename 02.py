...
valid = 0
for r in l:
    limits, letter, password = r.split(" ")
    count = password.count(letter[0])
    lower, higher = [int(x) for x in limits.split("-")]
...
def check(word, index, character):
    if index > len(word) or word[index-1] != character:
...
for r in l:
    limits, letter, password = r.split(" ")
    lower, higher = [check(password, int(x), letter[0]) for x in limits.split("-")]
    if (not (lower and higher)) and (lower or higher):
        print(r)
        valid += 1
print(valid)