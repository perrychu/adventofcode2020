...
def check(k,v):
    if k == "byr":
        return isyear(v) and int(v) >= 1920 and int(v) <= 2002
    elif k == "iyr":
        return isyear(v) and int(v) >= 2010 and int(v) <= 2020
    elif k == "eyr":
        return isyear(v) and int(v) >= 2020 and int(v) <= 2030
    elif k == "hgt":
        if v[-2:] == "cm":
            return  v[:-2].isdigit() and int(v[:-2]) >= 150 and int(v[:-2]) <= 193 
        elif v[-2:] == "in":
            return v[:-2].isdigit() and int(v[:-2]) >= 59 and int(v[:-2]) <= 76
        else:
            return False
    elif k == "hcl":
        return v[0] == "#" and re.match(r"[0-9a-f]{6}",v[1:]) is not None
    elif k == "ecl":
...
#print(check("hcl","#12346"))

valid = 0
for passport in l:
    data = parse(passport)
    if all([(f in data and check(f,data[f])) for f in fields]):
        valid += 1
    else:
        print(data)
        print([(f,(f in data and check(f,data[f]))) for f in fields])
