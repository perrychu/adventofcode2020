s = '''striped tan bags contain 2 light silver bags, 1 drab black bag, 2 clear tan bags, 2 mirrored tan bags.
...
def parse(line):
    k = re.match(r"([a-z]+ [a-z]+) bags contain",line).groups()[0]
    v = re.findall(r"(\d+) ([a-z]+ [a-z]+) bag",line)
    return (k,v)
...
for line in s.split("\n"):
    k,v = parse(line)
    forward[k] = {color:num for num,color in v}
    for num,color in v:
        backward[color].append(k)

reachable = set()
def explore_backward(color):
    if color in reachable:
        return
    else:
        reachable.add(color)
        for c in backward[color]:
            explore_backward(c)

for c in backward["shiny gold"]:
    explore_backward(c)
...
def count_forward(color):
    if len(forward[color]) == 0:
        return 1
    sum = 0
    for bag in forward[color]:
        sum += int(forward[color][bag]) * count_forward(bag) 
    sum += 1
    print(color, forward[color], sum)
    return sum

print(count_forward("shiny gold")-1)