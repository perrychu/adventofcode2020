...
l = s.split("\n\n")
rules = l[0].splitlines()
cases = l[1].splitlines()
...
for line in rules:
    ...
    if rule == '"a"':
        ...
        for option in rule.split("|"):
            parts = option.split()
            value.append(tuple(parts))
    rule_dict[key] = value

def check_rule(rule, text):
    options = rule_dict[rule]
    remaining_options = []
    for part in options:
        result, text_remain = check_sequence(part, text)
        if result:
            remaining_options.extend(text_remain)
    if len(remaining_options) > 0:
        return True, remaining_options
    return False, None

def check_sequence(seq, text):
    if len(seq) == 0:
        return True, [text]
    
    if seq[0].isalpha():
        if seq[0] == text[:len(seq[0])]:
            return check_sequence(seq[1:], text[len(seq[0]):])
        else:
            return False, None
    elif seq[0].isdigit():
        result, remaining_text = check_rule(seq[0], text)
        if result:
            remaining_options = []
            for text_remain in remaining_text:
                result, text_remain = check_sequence(seq[1:], text_remain)
                if result:
                    remaining_options.extend(text_remain)
            if len(remaining_options) > 0:
                return True, remaining_options
