...
def map_allergens(lines):
    allergen_map = defaultdict(list)
    for line in lines:
        igs, ags = parse(line)
        for ag in ags:
            allergen_map[ag].append(igs)
    return allergen_map

def filter_common_ingredients(allergen_map):
    common_ingredients = {}
    for ag in allergen_map:
        common_igs = set(allergen_map[ag][0])
        for igs in allergen_map[ag][1:]:
            common_igs.intersection_update(set(igs))
        common_ingredients[ag] = common_igs
    return common_ingredients
            
allergen_map = map_allergens(l)
allergen_ingredients = filter_common_ingredients(allergen_map)

potential_allergens = set()
for x in allergen_ingredients:
    potential_allergens.update(allergen_ingredients[x])

count = 0
for line in l:
    igs, _ = parse(line)
    for i in igs:
        if i not in potential_allergens:
            count += 1
...
def match_allergen_ingredients(common_ingredients):
    solved = {}
    assigned = set()
    while len(assigned) < len(common_ingredients):
        for ag in common_ingredients:
            common_ingredients[ag] = common_ingredients[ag].difference(assigned)

            if len(common_ingredients[ag]) == 1:
                ig = common_ingredients[ag].pop()
                solved[ag] = ig
                assigned.add(ig)
    return solved

print("Part 2")
allergen_matches = match_allergen_ingredients(allergen_ingredients)

print(','.join([allergen_matches[x] for x in sorted(allergen_matches.keys())]))
